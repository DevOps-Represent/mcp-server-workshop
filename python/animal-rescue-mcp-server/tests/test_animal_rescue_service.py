"""Unit tests for AnimalRescueService."""

from datetime import datetime

import pytest

from src.animal_rescue_service import AdoptionCertificate, Animal, AnimalRescueService
...
class TestAnimalRescueService:
    ...
    def test_adopt_animal_already_adopted(self, animal_service):
        ...
        # First adoption
        certificate1 = animal_service.adopt_animal("dog-001")
        assert certificate1 is not None

        # Second adoption attempt should fail
        certificate2 = animal_service.adopt_animal("dog-001")
    ...
    def test_list_animals_excludes_adopted(self, animal_service):
        ...
        # Adopt an animal
        animal_service.adopt_animal("dog-001")
        ...
        assert "dog-001" not in animal_ids
    ...
    def test_service_initialization(self, animal_service):
...
class TestAnimalRescueServiceIntegration:
    ...
    def test_full_adoption_workflow(self, animal_service):
        ...
        # 1. List all animals
        all_animals = service.list_animals()
        initial_count = len(all_animals)

        # 2. Find a specific animal
        target_animal = service.get_animal_by_name("Max")
        assert target_animal is not None
        target_id = target_animal["id"]

        # 3. Get animal by ID
        same_animal = service.get_animal_by_id(target_id)
        assert same_animal == target_animal

        # 4. Adopt the animal
        certificate = service.adopt_animal(target_id)
        assert certificate is not None
        assert certificate["animal_id"] == target_id

        # 5. Verify animal is no longer available
        updated_animals = service.list_animals()
        assert len(updated_animals) == initial_count - 1

        # 6. Verify animal is marked as adopted
        adopted_animal = service.get_animal_by_id(target_id)
        assert adopted_animal["adopted"] is True

        # 7. Verify cannot adopt again
        second_certificate = service.adopt_animal(target_id)
